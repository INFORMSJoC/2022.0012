...
%params.BarlterLimit = 1000; % Barrier iteration limit; default:1000
params.TimeLimit = 7200; % Time limit; default: infinity
%%
params.BarConvTol = 1e-8; % Barrier convergence tolerance; default: 1e-8 (relative gap)
params.FeasibilityTol = 1e-8; % primal feasibility tol 
params.OptimalityTol = 1e-8;  % dual feasiblility tol
%%

%params.Presolve = 0; % Presolve level is off; default: -1 (auto)

params.TuneOutput = 3; % Tuning output level (2+detailed solver output); 
                       % default: 2 (summary output + for each parameter set tried)
